    ...
    >
        ...
        <div class="flex h-14 items-center justify-between px-4">
            <div class="flex items-center gap-2.5">
                <span class="text-[15px] font-semibold text-[#E0E0DE]">{{ appName }}</span>
        ...
        <div class="px-3 pb-3">
            <div class="rounded-xl border border-[#2b2b2b] bg-[#252525] transition-colors focus-within:border-[#3a3a3a]">
                <input
                    ...
                    class="w-full bg-transparent px-4 py-3 text-[13px] text-[#E0E0DE] outline-none placeholder:text-[#666666]"
                />
                <div class="flex items-center justify-between px-3 pb-3">
                    ...
                    <button
                        ...
                        :class="composerValue ? 'bg-[#C4856F] text-[#141414]' : 'bg-[#C4856F] text-[#141414] opacity-60'"
                    >
                        <Send class="h-4 w-4" />
                    </button>
                </div>
                    ...
                    <PopoverContent :side-offset="8" align="start" class="w-[280px] rounded-xl border border-[#2b2b2b] bg-[#202020] shadow-2xl">
                        <div class="border-b border-[#2b2b2b] p-2">
                            <div class="flex items-center gap-2 rounded-lg bg-[#1b1b1b] px-3 py-2">
                                ...
                                <input v-model="repoSearchQuery" type="text" placeholder="Search repositories" class="flex-1 bg-transparent text-[13px] text-[#E0E0DE] outline-none placeholder:text-[#666666]" />
                            </div>
                        </div>
                        <div class="max-h-[280px] overflow-y-auto p-1.5">
                            ...
                            >
                                <div class="flex flex-col items-start">
                                    <span class="text-[13px] font-medium text-[#E0E0DE]">{{ repo.name }}</span>
                                    <span class="text-[11px] text-[#666666]">{{ repo.owner }}</span>
                                </div>
                                <Check v-if="repo.selected" class="h-4 w-4 text-[#C4856F]" />
                            </button>
                        </div>
                        <div class="border-t border-[#2b2b2b] p-2">
                            ...
                            <button class="flex w-full items-center justify-center gap-2 rounded-lg bg-[#2b2b2b] py-2.5 text-[13px] font-medium text-[#E0E0DE] transition-colors hover:bg-[#333333]">
                                <Github class="h-4 w-4" />
                                Install GitHub App
                            </button>
                        </div>
                ...
                <PopoverPortal>
                    <PopoverContent :side-offset="8" align="start" class="w-[200px] rounded-xl border border-[#2b2b2b] bg-[#202020] shadow-2xl">
                        <div class="border-b border-[#2b2b2b] p-2">
                            <div class="flex items-center gap-2 rounded-lg bg-[#1b1b1b] px-3 py-2">
                                <Search class="h-4 w-4 text-[#666666]" />
                                <input v-model="branchSearchQuery" type="text" placeholder="Search branches" class="flex-1 bg-transparent text-[13px] text-[#E0E0DE] outline-none placeholder:text-[#666666]" />
                            </div>
                        </div>
                        <div class="max-h-[200px] overflow-y-auto p-1.5">
                            ...
                            >
                                <span class="text-[13px] text-[#E0E0DE]">{{ branch.name }}</span>
                                <Check v-if="branch.selected" class="h-4 w-4 text-[#C4856F]" />
                            </button>
                        </div>
                    </PopoverContent>
                </PopoverPortal>
                ...
                <DropdownMenuPortal>
                    <DropdownMenuContent :side-offset="4" align="end" class="min-w-[120px] rounded-lg border border-[#2b2b2b] bg-[#202020] p-1 shadow-xl">
                        <DropdownMenuItem
                            ...
                            :class="filterOption === option ? 'text-[#E0E0DE]' : 'text-[#a1a1aa]'"
                            @click="filterOption = option"
                        >
                            {{ option }}
                            <Check v-if="filterOption === option" class="h-3.5 w-3.5 text-[#C4856F]" />
                        </DropdownMenuItem>
                    </DropdownMenuContent>
                </DropdownMenuPortal>
            </DropdownMenuRoot>
        ...
        <div class="flex-1 overflow-y-auto px-2">
            <button
                ...
                :class="session.active ? 'border-l-2 border-[#C4856F] bg-[#202020]' : ''"
                @click="emit('selectSession', session.id)"
            >
                <div class="flex w-full items-start justify-between gap-2">
                    <span class="line-clamp-2 text-[13px] font-medium" :class="session.active ? 'text-[#E0E0DE]' : 'text-[#c4c4c4]'">
                        {{ session.title }}
                    </span>
                    <span v-if="session.active" class="mt-1 h-1.5 w-1.5 flex-shrink-0 rounded-full bg-[#C4856F]" />
                </div>
                <div class="mt-1.5 flex items-center gap-1.5">
                    ...
                    <template v-if="session.metrics">
                        <span class="text-[11px] text-[#666666]">·</span>
                        <span class="text-[11px] text-[#6B9E7A]">+{{ session.metrics.additions }}</span>
                        <span class="text-[11px] text-[#C97272]">-{{ session.metrics.deletions }}</span>
        ...
        <PopoverRoot>
            <PopoverTrigger as-child>
                <button class="flex w-full items-center gap-3 border-t border-[#2b2b2b] px-4 py-3 transition-colors hover:bg-white/[0.03]">
                    <div class="flex h-7 w-7 items-center justify-center rounded-full bg-[#C4856F] text-[11px] font-semibold text-[#141414]">
                    ...
                    <div class="my-1 border-b border-t border-[#2b2b2b] py-1">
                        ...
                        >
                            ...
                            <div class="flex flex-1 flex-col items-start">
                                <span class="text-[13px] font-medium text-[#E0E0DE]">{{ workspace.name }}</span>
                                <span class="text-[11px] text-[#666666]">{{ workspace.plan }}</span>
                            </div>
                            <Check v-if="workspace.selected" class="h-4 w-4 text-[#C4856F]" />
                        </button>
                    </div>
